...
**Notes on the code as written**
A few things I want to flag if I reuse or hand off this notebook. My plot_hist helper draws the mean line twice instead of drawing both mean and median, so the median marker never actually appears — cosmetic only, doesn't affect the analysis. My correlation heatmap's format string 'fmt='.21'' is not a valid matplotlib float format and will raise an error if I re-run that cell as-is; '.2f' is almost certainly what I meant. My final prediction cell calls model.predict rather than loaded_model.predict, which will fail in a fresh session where only loaded_model exists (model is left over from the earlier training cell in the same runtime). My file path for the source CSV is a Google Drive path specific to my original Colab environment and will need to be swapped out to run elsewhere.
